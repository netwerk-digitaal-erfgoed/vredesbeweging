...
with open('data/Vredesbeweging.tsv', mode='r') as csv_file:
    ...
    for row in csv_reader:
        ...
        if row["title"]:
            ...
            print(f'{uri} <http://schema.org/name> "{title}" .')
            ...
            if row[f"creato {i}"]:
                ...
                for creat in creatos:
                    ...
                    if row[f"creato uri {i}"]:
                        ...
                        print(f'{uri} <http://schema.org/creator> <{curi}> .')
                        ...
                        print(f'{uri} <http://schema.org/creator> _:creator_{creatorid} .')
        ...
        if row["subjec"]: 
            ...
            print(f'{uri} <http://schema.org/about> "{subjec}" .')
            
        if row["descri"]: 
            descri=row["descri"].replace('"', r'\"').replace('\n', r'\\n');
            print(f'{uri} <http://schema.org/abstract> "{descri}" .')

        if row["losse"]: 
            losse=row["losse"].replace('"', r'\"').replace('\n', r'\\n');
            print(f'{uri} <http://schema.org/abstract> "{losse}" .')

        if row["langua"]: 
            languas=row["langua"].split(";")
            for langua in languas:
                lang=langua.strip() 
                print(f'{uri} <http://schema.org/inLanguage> _:lang_{lang} .')

        if row["publis"]: 
            publis=row["publis"].replace('"', r'\"');
            print(f'{uri} <http://schema.org/publisher> "{publis}" .')
            # TODO zou eigenlijk Organization moeten zijn
            
        if row["source"]: 
            source=row["source"].replace('"', r'\"');
            print(f'{uri} <http://schema.org/sourceOrganization> "{source}" .')
            # TODO zou eigenlijk Organization moeten zijn
        ...
        if row["date"]:
            ...
            print(f'{uri} <http://schema.org/dateCreated> "{date}" .')

        if row["contri"]: 
            contri=row["contri"].replace('"', r'\"').replace('\n', r'');
            print(f'{uri} <http://schema.org/contributor> "{contri}" .')
            # TODO zou eigenlijk Organization moeten zijn
            
        if row["identi"]: 
            identi=row["identi"].replace('"', r'\"');
            print(f'{uri} <http://schema.org/identifier> "{identi}" .')
        ...
        print(f'{uri} <http://schema.org/image> <https://cdm21069.contentdm.oclc.org/digital/api/singleitem/collection/p21069coll13/id/{dmrecord}/thumbnail> .')
        print(f'{uri} <http://schema.org/mainEntityOfPage> <https://cdm21069.contentdm.oclc.org/digital/collection/p21069coll13/id/{dmrecord}> .')
        print(f'{uri} <http://schema.org/url> <https://cdm21069.contentdm.oclc.org/digital/api/collection/p21069coll13/id/{dmrecord}/download> .')
