...
base = 'https://cdm21069.contentdm.oclc.org/digital/bl'
port = ''
collections = {}
...
if __name__ == "__main__":
    # a single page item
    single = item('p21069coll13', '1') # , pageinfo='on'
#    f = CSV('myfile.csv', header=['foo', 'bar', 'one', 'two'])
#    f.writerow(single)
#    f.close()
    ...
    # 'format' # pagina's bij sommige
    # 'type' # 1 Book, 1 Map, rest geen waarde
    # 'fullte', te veel ruis
    f = CSV("export.csv", header=fields)
    
    vredesbeweging = Collection('p21069coll13')
    vredesbeweging.getItems()
    for i in vredesbeweging.items:
        single = item('p21069coll13', i) 
        row = []
        for p in fields:
            row.append(single.info[p])
